...
// @version      153.2
// @description  사용자가 등록한 아프리카티비 숲 스트리머의 방송 상태를 확인하여 알림을 제공합니다.
// @author       che_dd_hyuji
...
(async function() {
    ...
    // 전역 방송 상태 관리 함수 (탭 간 공유)
    async function updateBroadcastState(broadcasterId, isOnline) {
        const stateKey = "broadcastState";
        let state = await GM.getValue(stateKey, {});
                    ...
                    try {
                        ...
                        if (chan.RESULT === 1) {
                            resolve({
                                online: true,
                                title: chan.TITLE || '',
                                nickname: chan.NICKNAME || '' // 스트리머 이름(별명)
                            });
                        } else {
